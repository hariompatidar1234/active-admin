...
RSpec.describe 'Categories', type: :request do
  ...
  describe 'PUT /categories/:category_id' do
    ...
    it 'updates a category for owners' do
      put "/categories/#{category.id}", params: { name: 'maxicon' },
                                        headers: { 'Authorization' => "Bearer #{jwt_encode(user_id: owner.id)}" }
      expect(response).to have_http_status(:ok)
    end
    it 'returns an error for invalid category update' do
      put "/categories/#{category.id}", params: { name: nil },
                                        headers: { 'Authorization' => "Bearer #{jwt_encode(user_id: owner.id)}" }
      expect(response).to have_http_status(:unprocessable_entity)
    end
  end
