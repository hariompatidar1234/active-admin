...
RSpec.describe OrdersController, type: :request do
  ...
  describe "GET /orders" do
    it "order items list" do
      get orders_path, headers: { 'Authorization' => "Bearer #{jwt_encode(user_id: @user.id) }" }
  ...
  describe 'POST /orders' do
    it 'creates a new order and clears the cart' do
      post orders_path, params: { address: '123 Main St' }, headers: { 'Authorization' => "Bearer #{jwt_encode(user_id: @user.id) }" }
      expect(response).to have_http_status(:created)
    end
    it 'returns unprocessable entity status' do
      post orders_path, params: { address: nil }, headers: { 'Authorization' => "Bearer #{jwt_encode(user_id: @user.id) }" }
  ...
  describe 'GET /orders/:id' do
    it 'returns the order for the current user' do
      get order_path(1), headers: { 'Authorization' => "Bearer #{jwt_encode(user_id: @user.id) }" }
      expect(response).to have_http_status(:ok)
    end
  end
